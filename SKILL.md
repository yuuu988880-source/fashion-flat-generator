...
`fashion_flats_full_pipeline.py` 只准备任务与提示词，不自带视觉识别或图像生成模型。宿主先看图并写 observation JSON，再按需运行：

```bash
python3 fashion_flats_full_pipeline.py --image /path/model.png --observations /path/observations.json --out /path/job
python3 fashion_flats_full_pipeline.py --image /path/model.png --observations /path/observations.json --line-master /path/front_line.png --out /path/color-job
```

脚本校验输入、复制图片、生成 prompt 与哈希清单；上色任务必须有母版。字段格式见仓库 `examples/observation.example.json`（虚构示例，必须替换）。命令成功不代表图像已生成或质量已通过。
